      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 sm:p-8">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-gray-900">{t("markAsSolvedTitle")}</h2>
          </div>
          <button
            onClick={onCancel}
            disabled={saving}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className={labelClass}>{t("solutionSummary")}</label>
            <textarea
              required
              rows={4}
              value={solutionText}
          ...
          <div>
            <label className={labelClass}>
              {t("solutionImage")} <span className="text-gray-400 font-normal">{t("optional")}</span>
            </label>
          ...
          <div className="pt-2 flex gap-3">
            ...
            >
              {saving ? (
                ...
                <><CheckCircle2 className="w-4 h-4" /> {t("confirmSolved")}</>
              )}
            </button>
          </div>
        </form>
