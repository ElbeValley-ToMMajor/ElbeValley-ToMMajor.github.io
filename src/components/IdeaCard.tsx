...
export function IdeaCard({ idea, onVote, onOpen, userVote = 0, rank }: IdeaCardProps) {
  ...
  return (
    <div
      onClick={() => onOpen(idea)}
      className="bg-white rounded-xl border border-green-100 shadow-sm hover:shadow-md hover:border-green-300 transition-all duration-200 p-4 sm:p-5 flex items-start gap-3 sm:gap-4 cursor-pointer group"
    >
      {/* Rank */}
